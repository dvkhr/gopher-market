...
type accrual struct {
	...
	User_id      string    `json:"user_id"`      // уникальный идентификатор пользователя
	Order_number int       `json:"number"`       // номер заказа
	Accrual      float64   `json:"accrual"`      // сумма начисления
	Processed_at time.Time `json:"processed_at"` // дата последнего обновления баланса time.RFC3339
...
type withdrawals struct {
	...
	User_id      string    `json:"user_id"`      // уникальный идентификатор пользователя
	Order_number int       `json:"number"`       // номер заказа
	Withdrawn    float64   `json:"withdrawn"`    // сумма списания
	Processed_at time.Time `json:"processed_at"` // дата последнего обновления баланса time.RFC3339
